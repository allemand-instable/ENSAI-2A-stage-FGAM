...
\idee{
L'idée c'est que si l'on dispose de covariables $X = ( \underbracket{X_1, \dots, X_k}_{X_{lin}}, \underbracket{X_{k+1}, \dots, X_d}_{X_{\neg lin}})$ alors au lieu d'estimer :
...
\begin{equation*}
	\begin{array}{rcl}
		\widehat m(X) & = & X_{lin}^T \, \widehat \beta + \grandop{ n^{-1/2}} + \widehat g(X_{\neg lin}) + \grandop{ n^{-\frac k {2k + (d-k)}} }
		\\
		              & = & m(X) + \grandop{\underbracket{n^{-\frac k {2k + (d-k)}}}_{\textsf{pire vitesse}}}
	\end{array}
\end{equation*}
